...
c
 99   return
     ...
     1                      nvar,val,time,hx,hy,qx,qy,irr,lstgrd, i, j)
      ...
      dimension r(4), s(4), ww(4), rtri(3), stri(3), wtri(3)
...
!            endif
       ...
       r = (/-dsqrt(1.d0/3.d0),  dsqrt(1.d0/3.d0),
     .        dsqrt(1.d0/3.d0), -dsqrt(1.d0/3.d0)/)
       s = (/ -dsqrt(1.d0/3.d0), -dsqrt(1.d0/3.d0),
     .         dsqrt(1.d0/3.d0),  dsqrt(1.d0/3.d0) /)
...
!           endif
  ...
  21      continue ! for each triangle
        ...
        do 11 iq = 1, 4
          xval = xcen1 * (1.d0 - r(iq))/2. + xcen3 * (1.d0 + r(iq))/2.
          yval = ycen1 * (1.d0 - s(iq))/2. + ycen3 * (1.d0 + s(iq))/2.

          call f(state, xval, yval, time, iprob)

          val(i,j,1) = val(i,j,1) + ww(iq) * state(1)/ 4.d0
          val(i,j,2) = val(i,j,2) + ww(iq) * state(2)/ 4.d0
          val(i,j,3) = val(i,j,3) + ww(iq) * state(3)/ 4.d0
          val(i,j,4) = val(i,j,4) + ww(iq) * state(4)/ 4.d0
 11   continue
...
!          dnx = cos(dangle)
!          dny = sin(dangle)
!          xi = (dnx*(x+0.25d0) + dny*(y+0.25d0))/(time+1.d0)
!            ! could be time + 0.075d0
!
!          rhol = 8.d0
!          ul = 8.25d0
!          pl = 116.5d0
...
!          state(2) = rhot * ut
!          state(3) = rhot * vt
!          state(4) = pt/(gamma-1) + 0.5d0*rhot*(ut**2 + vt**2) ;

!          state(1) = 1.d0 + xi
!          if(quad) then !convert to primitive for p = 1
!          state(1) = rhot
!          state(2) = ut
!          state(3) = vt
!          state(4) = pt
