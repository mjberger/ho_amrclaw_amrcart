...
!        endif
  ...
  21      continue ! for each triangle
      ...
      if( (ssw .eq. 2  .or. ssw .eq. -2) .and. ihob .eq. 1) then
      ...
      elseif((ssw .eq. 3  .or. ssw .eq. -3) .and. ihob .eq. 1) then
      x1 = poly(ivert-2,1,kirr)
      y1 = poly(ivert-2,2,kirr)
      x2 = bdry(1,1,kirr)
      y2 = bdry(1,2,kirr)
 ...
 11   continue
      ...
      subroutine f(state, x, y, time, iprob)
        ...
        if(iprob .eq. 28) then
          ...
          rhot = (1.d0 + 1.0125d0*(1.d0 - r**(-2.d0)))**2.5d0
          ut = (2.25d0/r)*sin(theta)
          vt =-(2.25d0/r)*cos(theta)
          pt = (1.d0/gamma)*rhot**gamma

          state(1) = rhot
          state(2) = rhot * ut
          state(3) = rhot * vt
          state(4) = pt/(gamma-1.d0) + 0.5d0*rhot*(ut**2.d0 + vt**2.d0)
