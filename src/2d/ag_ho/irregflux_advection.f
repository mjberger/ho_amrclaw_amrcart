...
c     
       ...
       call exact(xlow,xright,ylow,ytop,level,mitot,mjtot,
     1               4,val,time,hx,hy,qx,qy,irr,lstgrd,bxpt,bypt)

      call getdir(dir)

c     (alf,beta) is inward pointing
      dot = alf*dir(1) +beta*dir(2)

c     upwinding
c     
         firreg(k,1) = ( 0.5d0*dot * ( val(1) + rho )
     .            +0.5d0*abs(dot)*( val(1) - rho  ) )*rlen
         firreg(k,2) = 0.d0
         firreg(k,3) = 0.d0
         firreg(k,4) = 0.d0
