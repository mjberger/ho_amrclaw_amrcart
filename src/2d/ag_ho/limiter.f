    ...
    ! barth jespersen on cut cells, recentering on whole cells
      subroutine limiter_a(qp, qx, qy, mitot, mjtot, irr, nvar, hx, hy,
...
!            if (ioff .eq. 0 .and. joff .eq. 0) go to 31
            if (outside(i+ioff,j+joff)) go to 31
...
!             if (ioff .eq. 0 .and. joff .eq. 0) go to 32 ! no eqn to solve
             if (outside(i+ioff,j+joff)) go to 32
             koff = irr(i+ioff,j+joff)
...
! barth jespersen on cut cells and on whole cells
     ...
     .            WHEN RECONSTRUCTING IN PRIMITIVE VARIABLES."
      ...
      do 50 j = 1, mjtot
        ...
        k = irr(i,j)
        if ((k .eq. -1) .or. (inuf(i,j) .eq. 0)) cycle
...
c
      ...
      do 30 i = 1, mitot
          k = irr(i,j)
          if ((k .eq. -1) .or. (inuf(i,j) .eq. 1)) cycle ! skips solid/regular cells
