...
c      solve for Lagrange multiplier lambda: A_k^T lambda = c
          ...
          lambda(1) = (1/det)*(A_k(2,2)*c(1)-A_k(2,1)*c(2))
          lambda(2) = (1/det)*(-A_k(1,2)*c(1)+A_k(1,1)*c(2))
          if (output_debug) then
             write(*,*) "lambda", lambda
          endif
...
c         Calculate step direction by solving A_k p_k = e_q
          ...
          p_k(1) = (1/det)*(A_k(2,2)*e_q(1)-A_k(1,2)*e_q(2))
          p_k(2) = (1/det)*(-A_k(2,1)*e_q(1)+A_k(1,1)*e_q(2))
          if (output_debug) then
             write(*,*) "p_k", p_k
          endif
...
c         Calculate step length
          ...
          do 20 i=1,row_A
             ...
             if (output_debug) then
               write(*,*) "potential constraint violation for ",i,
     .               " is ",test
             endif
c    this next commented out line is to try if lowering the eps test still fails
c             if (test .LT. -1.E-11 .and. i .ne. W(1) .and. i .ne. W(2)) then
             if (test .LT. -1.E-11) then
                size_D_k = size_D_k + 1
