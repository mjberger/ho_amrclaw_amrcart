...
c -------------------------------------------------------------
c
      subroutine vrm_conserved(qr,ql,rx,ixmin,ixmax,iflip,msize)
...
c
      ...
      do 10 k = ixmin, ixmax
         rhor(k) = qr(k,1)
         ur(k)   = qr(k,inorm)/qr(k,1)
         utr(k)  = qr(k,itan)/qr(k,1)
         pr(k)   = (gamma-1.d0)*(qr(k,4)
     .                           -0.5d0*rhor(k)*(ur(k)**2+utr(k)**2) )
         rhol(k) = ql(k,1)
         ul(k)   = ql(k,inorm)/ql(k,1)
         utl(k)  = ql(k,itan)/ql(k,1)
         pl(k)   = (gamma-1.d0)*(ql(k,4)
     .                           -0.5d0*rhol(k)*(ul(k)**2+utl(k)**2) )
  10     continue
c
      do 20 k = ixmin, ixmax
...
c       sR = ur(k) + aR; 
     ...
     .                  pr(k)/gamma1
         ...
         stateL(1)     = ql(k,1)
         stateL(inorm) = ql(k,inorm)
         stateL(itan)  = ql(k,itan)
         stateL(4)     = ql(k,4)

         stateR(1)     = qr(k,1)
         stateR(inorm) = qr(k,inorm)
         stateR(itan)  = qr(k,itan)
         stateR(4)     = qr(k,4)
