...
c ---------------------------------------------------------------
c
        subroutine filpatch(xlp,xrp,ybp,ytp,level,nvar,valbig,time,
     &                      intime,mitot,mjtot,nrowst,nrowend,ncolst,
     &                      ncolend,irr)
...
c and inserting them in the enlarged array valbig. It first fills in
...
c is either contained in a level 1 grid or exterior to the computational domain.
	...
	ifill = 1
     ...
     &  valbig,mitot,mjtot,time,intime,ifill,locuse(level))
...
20      continue
     ...
     &  (alloc(locuse(levf) + iff - 1 + (jf - 1)*nrow(levf)).eq.0.)
		...
		ir00 = ialloc(iirr(ic,jc,levc)) 
              	ir10 = ialloc(iirr(ic+1,jc,levc))
             	ir01 = ialloc(iirr(ic,jc+1,levc))
             	ir11 = ialloc(iirr(ic+1,jc+1,levc))
		if ((ir00 .ne. -1) .and.
     .        	    (ir10 .ne. -1) .and.
     .       	    (ir01 .ne. -1) .and.
     .       	    (ir11 .ne. -1)) then
		  allin = .true.
		    ...
		    if (levf .eq. level) then
                         if  (irr(iff+nrowst-1,jf+ncolst-1).ne.-1) then
			     reset = .true.
			 else
			     reset = .false.
			 endif
...
c                    # reset iclose,jclose to extrapolate soln
			   ...
			   iclose = ic
			   jclose = jc
		        else if (ir10 .ne. -1) then
		           iclose = ic+1
			   jclose = jc
		        else if (ir01 .ne. -1) then
			   iclose = ic
			   jclose = jc+1
		        else if (ir11 .ne. -1) then
			   iclose = ic+1
			   jclose = jc+1
		        else
			   write(6,900)
