...
c
     ...
     .                      ffluxlen,gfluxlen)
       ...
       logical IS_OUTSIDE, INDEX_OUTSIDE, NOT_OK_GHOST
...
c never use cells from exterior to the domain for SRD
     ...
     .                    y .lt. ylower .or. y .gt. yupper)
       ...
       INDEX_OUTSIDE(i,j) = (i .lt. 1 .or. i > mitot .or.
     .                       j .lt. 1 .or. j > mjtot)
...
c       form qMerge vals 
        ...
        do 10 i = lwidth-1, mitot-lwidth+1
            ...
            if (IS_OUTSIDE(xc,yc)) then 
              qMerge(:,i,j) = 0.d0  !rinfinity ! to make sure we dont use it
              go to 10 
            endif
            if (k.eq.lstgrd) then
              qMerge(:,i,j) = q(:,i,j)
...
c
             ...
             do 27 joff = -nco, nco
             do 27 ioff = -nco, nco
                if (INDEX_OUTSIDE(i+ioff,j+joff)) go to 27
...
c        call limitTileGradientBJ(qmerge,gradmx,gradmy,xcentMerge,
...
c            calc value to contribute to neighboring cell i+ioff,j+joff
c            in cell i,j'n neighborhood
...
c            check if physical state
 ...
 900            format("should not happen, rho,pr,mptr,i,j,istage",
     .                  2e15.7,4i4)
                ...
                write(*,902) (qMerge(mm,i,j),mm=1,4)
 902            format("original qmerge vals",4e11.3)
 901            format("         reconstructing to offsets ",2i5,
     .                " with ",i5," nhoods")
             endif
c            add to nbor value appropriately weighted             
...
c
     ...
     .                     dx,dy,istage,mptr,ffluxlen,gfluxlen,areaMin)
     ...
     .                   y .lt. ylower .or. y .gt. yupper)
      ...
      INDEX_OUTSIDE(i,j)  = (i < 1 .or. i > mitot .or. 
     .                       j < 1 .or. j > mjtot)


      numHoods = 1  ! each cell starts off as its own neighborhood
      ncount = 0
      maxnco = 0

c     this code below counts on fact that don't need more than
c     2 cells to a side for a merging neighborhood
c     might include ghost cells here due to interior grid interfaces
c     but want to try to eliminates cells that are actually outside
c     the domain
      do 10 j = 3, mjtot-2
      do 10 i = 3, mitot-2
         k = irr(i,j)  
         if (k .eq. -1) go to 10
         if (k .eq. lstgrd) go to 10
         if (ar(k) .gt. areaMin) go to 10
         call getCellCentroid(lstgrd,i,j,xc,yc,
     &                         xlow,ylow,dx,dy,k)
         ...
         do while (vqmerge < areaMin) 
            do 15 joff = -nco, nco
            do 15 ioff = -nco, nco
               if (INDEX_OUTSIDE(i+ioff,j+joff)) go to 15  
               koff = irr(i+ioff,j+joff)
     ...
     &                              xlow,ylow,dx,dy,koff)
               ...
               ! test for 0 offsets below since numHoods initialized to 1 for all cells
               if (firstTimeThru .and. ioff.ne.0 .and. joff.ne.0) then ! add to their count
...
c IS THIS STILL A THING      
      ...
      if (maxnco .gt. 2) then
       ...
       if (maxnco .eq. 2) then
         write(*,*)"maxnco = ",maxnco," must fix code for nco > 1"
       endif
...
c           write(*,*)"found case of outside centroid. cell = ",i,j
            volMerge(i,j) = 0.d0
            go to 20
         endif
         ...
         do 25 joff = -nco, nco
         do 25 ioff = -nco, nco
            if (INDEX_OUTSIDE(i+ioff,j+joff)) go to 25
            koff = irr(i+ioff,j+joff)
