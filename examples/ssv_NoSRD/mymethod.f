...
c
         ...
         k = irr(i,j)
         dtmine = dtn*ar(k)/ar(lstgrd) ! not quite best local timestepping but try
         do m = 1, nvar
            resid(m) = (f(m,i+1,j)-f(m,i,j)+g(m,i,j+1)-g(m,i,j))
     &                 - firreg(m,k)
            res(m,i,j) = resid(m) ! for debugging save both
            if (istage .eq. 1) then
               q(m,i,j) = qold(m,i,j) - dtmine/ar(k)*resid(m)
            else
               q(m,i,j) = q(m,i,j) - dtmine/ar(k)*resid(m)  ! second stage builds on first
            endif
         end do
 917     continue
