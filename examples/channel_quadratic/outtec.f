...
c        # test again for kirr -1 in case want to view in tecplot
         if (kirr .eq. lstgrd) then
            ...
            ch = ' '
            call makep(poly(1,1,kirr),i-nghost,j-nghost,xlow,ylow,
     &                 dx,dy)
         else 
            xcen = xcirr(kirr)
            ycen = ycirr(kirr)
...
c        compute cell centered averaged used to computed cell centered error         
         call channelAvgInit(poly(1,1,kirr),xcen,ycen,rhot,ut,vt,pt) 
...
!         call channelPtInit(xcen,ycen,rhot,ut,vt,pt) 
!         ouptut pw const error at each corner
