...
c
     ...
     &          qy(nvar,mitot,mjtot),irr(mitot,mjtot)
      ...
      logical   prflag, quad, enufNbor, outside
      data      prflag/.false./
...
c     do 110 ix0 = lwidth-1, mitot-lwidth+2
c     do 110 iy0 = lwidth-1, mjtot-lwidth+2
         k = irr(ix0,iy0)
         if (k .eq. -1) go to 110
...
c  
 ...
 110  continue
      ...
      if (nolimiter) go to 111

      if (limitCell .eq. 2) then  ! print LP limiter stats
      ...
      else  ! for BJ limit whole grid at once
         call limiterBJ(qp,qx,qy,mitot,mjtot,irr,nvar,hx,hy,lwidth,
     &                lstgrd,xlow,ylow)
      endif
c
 111  if (prflag) then
